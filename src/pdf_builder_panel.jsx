  ...
  const getJSX = (component) => {
    ...
    const BuildPreview = ({data}) => {
        ...
        switch(data.type){
            ...
            default: 
            const variable = (data.data?.var ?? data.data?.text);
                return <Row>
                <Typography style={{fontWeight: (variable)?.[0] === "$" ?"normal":"bold", fontStyle: variable?.[0] === "$" ? "italic" :"normal"}}>{variable}</Typography>
                {(variable)?.[0] === "$" && <Chip style={{marginLeft: 5}} size="small" variant="outlined" label="variable"/>}
                </Row>
        }
    }
